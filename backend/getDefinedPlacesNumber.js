...
export default async function getDefinedPlacesNumber(humanId) {
    const queryText = `WITH all_interactions AS (SELECT places.place_name, places.category, places.latitude AS lat, places.longitude AS lng, COUNT(places.id) as place_count
    FROM visits
    ...
    GROUP BY places.id)
    SELECT COUNT(*) AS places_number FROM all_interactions
    `
    const [queryResult] = await pool.query(queryText, [humanId, humanId, humanId, humanId, humanId, humanId, humanId, humanId, humanId, humanId, humanId, humanId])
    const returnedValue = queryResult[0]["places_number"]
